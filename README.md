...
- 支持推理模型思考过程输出
- **QQ 群管理集成**（v1.2.0）：禁言、入群审批、自动审批策略、禁言状态查询
- **@提及识别**（v1.2.0）：AI 可直接使用 @成员 的用户名进行操作
- **群管理工具审批**（v1.2.0）：危险操作需管理员按钮审批

### WebUI 图形化配置（v1.2.0 新增）

- 浏览器访问 `http://127.0.0.1:5678` 管理所有配置项
- 暗色简约风格，分组展示全部配置（机器人、聊天格式、白名单、Agent 等）
- 启动时自动生成随机密码，日志打印一次；`/hb password` 修改
- `/hb webui` 查看地址
...
- **password** `<新密码>` —— 修改 WebUI 登录密码（仅管理员）
